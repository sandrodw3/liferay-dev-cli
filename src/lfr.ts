...
import { CMDS, isLiferayModule, PROFILES } from 'liferay'
...
const command = new Command()
	...
	.action(({ branch, clean, defaultOutput, module, skipDependencies }) => {
		if (!branch && !module && !isLiferayModule(Deno.cwd())) {
			throw new ValidationError(
				'You are not in a Liferay deployable module, please move to one or pass an option'
			)
		}
	...
	.option(
		'-b, --branch',
		`Format the current branch with \`${CMDS.formatBranch}\``,
		{
			conflicts: ['module'],
		}
	)
	...
	.action(({ branch, defaultOutput, module }) => {
		if (!branch && !module && !isLiferayModule(Deno.cwd())) {
			throw new ValidationError(
				'You are not in a Liferay formatable module, please move to one or pass an option'
			)
		}
	...
	.action(({ branch, defaultOutput, module }) => {
		if (!branch && !module && !isLiferayModule(Deno.cwd())) {
			throw new ValidationError(
				'You are not in a Liferay testable module, please move to one or pass an option'
			)
		}
