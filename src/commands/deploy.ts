...
export async function deploy({
	...
	if (!module && !branch) {
		await deployModule({ module: Deno.cwd(), options })
	}
...
async function deployBranchModules(options: Options) {
	...
	if (modules.length > 10) {
		if (
			!(await Confirm.prompt(
				'You are about to deploy more than 10 modules, do you want to continue?'
			))
		) {
			return
		}
	...
	description += `all modules modified in ${bold(
		blue(`[${branch}]`)
...
async function deploySelectedModule(options: Options) {
	...
	if (!isLiferayModule(module)) {
		log(
			`This is not a ${bold(
				blue('Liferay')
			)} deployable module, please select another one`
		)
